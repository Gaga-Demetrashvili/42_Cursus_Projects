...
/*   Updated: 2025/10/20 19:55:43 by gdemetra         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// Create a unique temp file, write heredoc lines, reopen for reading, unlink it
...
static void	run_command_child(t_ast *node, int heredoc_fd, int stdin_pre_set)
{
	int	fd;
	int	flags;
	...
	if (node->cmd->outfile)
	{
		flags = O_WRONLY | O_CREAT | (node->cmd->append ? O_APPEND : O_TRUNC);
...
static int	execute_pipe(t_ast *node)
{
	...
	if (node->left && node->left->type == TOKEN_WORD
		...
		left_heredoc_fd = collect_heredoc(node->left->cmd->heredoc);
	if (node->right && node->right->type == TOKEN_WORD
		&& node->right->cmd->heredoc)
		right_heredoc_fd = collect_heredoc(node->right->cmd->heredoc);
	pid_left = fork();
	if (pid_left == 0)
	{
